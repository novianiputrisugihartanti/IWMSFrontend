...
export default function TransaksiPengambilanBarangDetail({
  onChangePage,
  withID,
}) {
  ...
  useEffect(() => {
    const fetchHeaderData = async () => {
      ...
      try {
        const data = await UseFetch(
          API_LINK + "TransaksiPengambilanBarang/GetDataTranspengambilanById",
          { PabNo: withID }
        );
        if (data === "ERROR" || data.length === 0) {
          throw new Error(
            "Terjadi kesalahan: Gagal mengambil data pengambilan barang."
          );
        } else {
    ...
    const fetchTableData = async () => {
      ...
      try {
        const data = await UseFetch(
          API_LINK +
            "TransaksiPengambilanBarang/GetDataTranspengambilanDetailToShow",
          { PabNo: withID },
          "POST"
        );
